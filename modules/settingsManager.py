...
class settingsManagerApp:
    ...
    def launch(self): 
        ...
        preserve_file = tk.Checkbutton(
            ...
            command=lambda: self.update_system_setting("preserve_file", preserve_file_var.get())
        )
        ...
        saved_file_entry = tk.Entry(window, textvariable=saved_file_var, width=20)
        saved_file_entry.bind(
            "<FocusOut>", 
            lambda e: self.update_system_setting("saved_file_path", saved_file_var.get())
        )
        ...
        saved_file_entry.grid(row=1, column=1, sticky="w", padx=10, pady=10)
        saved_file_selector.grid(row=1, column=2, sticky="w", padx=10, pady=10)

    def browse_file(self, var):
        file_path = tk.filedialog.askdirectory(
            title="Select existing project file or directory in which to create a new one"
        )
        if file_path:
            var.set(file_path)
            self.update_system_setting("saved_file_path", file_path)

    def update_setting(self, key, value):
        self.pref.settings[key] = value
        self.pref.save_system_settings()

    def update_system_setting(self, key, value):
        self.pref.system_settings[key] = value
        self.pref.save_system_settings()